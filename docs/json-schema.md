...
- [`schemas/pooler-evidence.schema.json`](schemas/pooler-evidence.schema.json): normalized pooler evidence snapshot for `poolsim import pooler-evidence`.
...
```bash
npx ajv-cli validate \
  -s docs/schemas/pooler-evidence.schema.json \
  -d docs/fixtures/pooler-evidence.json
```
